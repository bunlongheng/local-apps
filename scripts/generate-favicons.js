...
async function generateForApp(appId, localPath) {
  ...
  const png32 = await sharp(png512).resize(SIZES.ico32, SIZES.ico32).ensureAlpha().png().toBuffer();
  const png16 = await sharp(png512).resize(SIZES.ico16, SIZES.ico16).ensureAlpha().png().toBuffer();
