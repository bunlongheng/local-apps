...
function run(suffix, minLines, minBranches, tests) {
  ...
  try {
    const r = spawnSync(process.execPath, ['--test', '--experimental-test-coverage', '--test-reporter=lcov', `--test-reporter-destination=${out}`, `--test-coverage-include=${suffix}`, ...tests], { stdio: ['ignore', 'ignore', 'inherit'] });
