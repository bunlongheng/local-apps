...
// --routes <file>: capture an explicit list of paths (source-discovery) instead
// of link-crawling. One path per line (e.g. /rag/search). First line = hub.
const routesIdx = args.indexOf('--routes');
const routesFile = routesIdx !== -1 ? args[routesIdx + 1] : null;
// --viewport: capture one viewport (above-the-fold) instead of the full scroll height
  ...
  for (let i = 0; i < links.length; i++) {
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
        ...
        await page.screenshot({ path: filePath, fullPage: !viewportOnly });
...
(async () => {
  ...
  if (routesFile && fs.existsSync(routesFile)) {
    ...
    links = fs.readFileSync(routesFile, 'utf8').split('\n').map(s => s.trim()).filter(Boolean).map(p => {
      const pathname = p.startsWith('/') ? p : '/' + p;
      const label = pathname === '/' ? 'home' : pathname.replace(/^\//, '').replace(/\//g, ' ');
      return { url: origin + pathname, pathname, label };
    });
    console.log('Using source route list (' + links.length + ' routes) from ' + routesFile);
  } else {
    links = await discoverLinks(discoverPage, url);
  }
