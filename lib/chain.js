...
async function runLevel(level, { id, uid, label, plistPath, port, dir, logPath, downMs = 0 }, deps) {
  ...
  if (level === 1) {
    ...
    // Bootstrap alone only loads a RunAtLoad=false plist; startCmd is what actually starts the
    // process (kickstart -k, falling back to enable + bootstrap + kickstart on a booted-out label).
    await exec(`${bootoutCmd(uid, label)}; sleep 1; ${startCmd(uid, label, plistPath)}`, { timeout: 15000 });
    ...
    const allowed = ['Read', 'Grep', 'Glob',
      `Bash(${startCmd(uid, label, plistPath)})`,   // the full start command; its fallback already bootstraps
