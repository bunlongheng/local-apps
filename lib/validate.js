...
// Health and local urls are fetched by the hub itself every 30s, so they must point at
// this machine: anything else turns the health loop into a blind SSRF / port-scan oracle.
function isLocalUrl(u) {
  try { const x = new URL(u); return (x.protocol === 'http:' || x.protocol === 'https:') && ['localhost', '127.0.0.1', '[::1]'].includes(x.hostname === '::1' ? '[::1]' : x.hostname); }
  catch { return false; }
}
function validateAppFields(body) {
  if (body.healthUrl != null && body.healthUrl !== '' && !isLocalUrl(body.healthUrl)) return 'healthUrl must be http(s)://localhost or 127.0.0.1 with a port';
  if (body.localUrl != null && body.localUrl !== '' && !isLocalUrl(body.localUrl)) return 'localUrl must be http(s)://localhost or 127.0.0.1 with a port';
...
// PWA name by how the dashboard was reached, so the installed icon says where it points.
function manifestLabel(host) {
  host = String(host || '');
  if (host.startsWith('100.')) return 'Apps (Tailscale)';
  if (host.startsWith('10.') || host.startsWith('192.168.')) return 'Apps (LAN)';
  if (host.split(':')[0].endsWith('.localhost')) return 'Apps (Caddy)';
  return 'Local Apps';
}

module.exports = { manifestLabel, isLocalUrl, isValidId, isSafePath, isSafeCommand, isSafeCaddyUrl, isSafeLaunchAgent, isSafeLaunchAgentPath, isSafeSegment, validateAppFields, xmlEscape };
