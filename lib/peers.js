...
const MAX_BODY = 1024 * 1024; // a peer status document is ~40 KB; anything near 1 MB is not a peer
function fetchJson(url, timeoutMs = 3000, maxBytes = MAX_BODY) {
  return new Promise((resolve, reject) => {
    const req = http.get(url, { timeout: timeoutMs }, (resp) => {
      let body = '', size = 0;
      resp.on('data', (c) => { size += c.length; if (size > maxBytes) { req.destroy(new Error('response too large')); return; } body += c; });
