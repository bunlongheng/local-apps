...
module.exports = function makeCaddy({ caddyfile, errorRoot, getLanIp: _getLanIp, exec, offlineSource = pathx.join(__dirname, '..', 'public', 'offline.html') }) {
  ...
  function reloadCaddy() {
    ...
    try {
      try { exec('caddy reload --config ' + caddyfile + ' --adapter caddyfile 2>/dev/null', { timeout: 15000 }); } catch (e) { console.warn(`  caddy reload failed: ${e.message}`); }
