...
function decorate(req, res) {
  ...
  req.path = decodeURIComponent(qIndex === -1 ? req.url : req.url.slice(0, qIndex));
  ...
  for (const method of ['get', 'post', 'put', 'delete', 'patch']) {
    app[method] = (pattern, fn) => {
      ...
      stack.push({
        fn,
        match: (req) => {
          if (req.method !== method.toUpperCase()) return null;
          const m = regex.exec(req.path);
          if (!m) return null;
          return Object.fromEntries(keys.map((k, idx) => [k, decodeURIComponent(m[idx + 1])]));
        },
      });
      return app;
    };
