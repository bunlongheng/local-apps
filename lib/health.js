...
// Factory deps: broadcast(fn) for SSE status pushes; timeoutMs per probe (tests shorten it).
const http = require('http');
const https = require('https');
const { execFile } = require('child_process');

module.exports = function makeHealth({ broadcast, timeoutMs = 3000 }) {
  const state = {};
  ...
  function probe(url, method) {
    return new Promise(resolve => {
      try {
        ...
        const req = mod.request(url, { method, timeout: timeoutMs, headers: { 'User-Agent': 'local-apps' } }, res => {
          res.destroy();
