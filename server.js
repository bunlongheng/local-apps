...
// Proxy: fetch a remote machine's /api/loops server-side (avoids CORS)
app.get('/api/machines/:id/loops', async (req, res) => {
  const m = db.getMachines().find(x => x.id === req.params.id);
  if (!m) return res.status(404).json({ error: 'machine not found' });
  const url = `http://${m.ip}:${m.port || 9875}/api/loops`;
  try {
    const data = await new Promise((resolve, reject) => {
      http.get(url, { timeout: 5000 }, (resp) => {
        let body = '';
...
// Proxy: a remote machine's /api/loops/log
app.get('/api/machines/:id/loops/log', (req, res) => {
  const m = db.getMachines().find(x => x.id === req.params.id);
  if (!m) return res.status(404).json({ error: 'machine not found' });
  const qs = require('url').parse(req.url).query || '';
  const url = `http://${m.ip}:${m.port || 9875}/api/loops/log?${qs}`;
  http.get(url, { timeout: 5000 }, (resp) => {
    let body = '';
    resp.on('data', c => body += c);
    resp.on('end', () => { res.status(resp.statusCode || 200).type('json').send(body); });
  }).on('error', (e) => res.status(502).json({ error: `unreachable: ${e.message}` }))
    .on('timeout', function () { this.destroy(); });
});

// Proxy: a remote machine's /api/loops/kill
app.post('/api/machines/:id/loops/kill', (req, res) => {
  const m = db.getMachines().find(x => x.id === req.params.id);
  if (!m) return res.status(404).json({ error: 'machine not found' });
  const payload = JSON.stringify(req.body || {});
  const opts = {
    hostname: m.ip, port: m.port || 9875, path: '/api/loops/kill', method: 'POST',
  ...
  r.on('error', (e) => res.status(502).json({ error: `unreachable: ${e.message}` }));
  r.on('timeout', () => r.destroy());
  r.write(payload);
  r.end();
});

// Proxy: a remote machine's /api/loops/interval
app.post('/api/machines/:id/loops/interval', (req, res) => {
  const m = db.getMachines().find(x => x.id === req.params.id);
  if (!m) return res.status(404).json({ error: 'machine not found' });
  const payload = JSON.stringify(req.body || {});
  const opts = {
    hostname: m.ip, port: m.port || 9875, path: '/api/loops/interval', method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(payload) }, timeout: 5000,
  };
  const r = http.request(opts, (resp) => {
    let body = '';
...
function refreshCronCache() {
  ...
  cronCache.data = CRON_JOBS.map(c => {
    ...
    // auto-restart reads from data/auto-restart.json
    let enabled;
    if (c.id === 'auto-restart') {
      try { enabled = JSON.parse(fs.readFileSync(path.join(__dirname, 'data', 'auto-restart.json'), 'utf8')).enabled; } catch { enabled = false; }
    } else {
      enabled = launchctlList.includes(label);
    }
    const result = { ...c, lastRun: null, lastLines: null, summaryData: null, enabled };
    try {
...
// Map every transcript by its sessionId (filename) -> path.
function transcriptIndex() {
  const root = path.join(os.homedir(), '.claude', 'projects');
  const idx = {};
  let projDirs = [];
  try { projDirs = fs.readdirSync(root); } catch { return idx; }
  for (const d of projDirs) {
    const pd = path.join(root, d);
    let files = [];
    try { files = fs.readdirSync(pd); } catch { continue; }
    for (const f of files) {
      if (f.endsWith('.jsonl')) idx[f.slice(0, -6)] = path.join(pd, f);
    }
  }
  return idx;
}

// Find the last ScheduleWakeup / CronCreate tool call in a transcript. Reads the
// whole file: a CronCreate is registered once and can sit anywhere in the log,
// so a tail scan would miss long-lived crons.
function lastLoopCall(fp) {
  let text;
  try { text = fs.readFileSync(fp, 'utf8'); } catch { return null; }
  if (!text) return null;
  let last = null;
  for (const line of text.split('\n')) {
    if (!line.includes('ScheduleWakeup') && !line.includes('CronCreate')) continue;
    let ev;
    ...
    for (const c of content) {
      if (c && c.type === 'tool_use' && (c.name === 'ScheduleWakeup' || c.name === 'CronCreate')) {
        last = { name: c.name, input: c.input || {}, ts: ev.timestamp };
      }
    }
  }
  return last;
...
// Optional friendly-name overrides for your own recurring launchd automations
// (interval- or calendar-scheduled com.local-apps.* agents). Key each entry by its
// plist label to give it a nicer name/description/log in the dashboard. Empty by
// default it ships with one example entry - add your own keyed by plist label.
// Any com.local-apps.*.plist you install is also auto-discovered below.
const LAUNCHD_LABELS = {
  'com.local-apps.example': { name: 'Example automation', prompt: 'Replace with your own recurring launchd job', log: path.join(os.homedir(), '.local-apps', 'logs', 'example.log') },
};
const { plistVal, calendarSchedule } = require('./launchd-parse');
function launchdLoops(now) {
  const dir = path.join(os.homedir(), 'Library', 'LaunchAgents');
  let files = [];
  try { files = fs.readdirSync(dir).filter(f => /^com\.local-apps\..*\.plist$/.test(f)); } catch {}
  const out = [];
  for (const f of files) {
    let xml; try { xml = fs.readFileSync(path.join(dir, f), 'utf8'); } catch { continue; }
    const interval = parseInt(plistVal(xml, 'StartInterval', 'integer') || '', 10);
    const schedule = calendarSchedule(xml);
    if (!interval && !schedule) continue; // only interval- or calendar-based jobs
    const label = plistVal(xml, 'Label', 'string') || f.replace(/\.plist$/, '');
    const meta = LAUNCHD_LABELS[label];
    if (!meta) continue; // only the curated automations
    const freshPath = meta.log || plistVal(xml, 'StandardOutPath', 'string');
    let age = 999999;
    if (freshPath) { try { age = now - Math.floor(fs.statSync(freshPath).mtimeMs / 1000); } catch {} }
    let enabled = false;
    try { require('child_process').execSync(`launchctl print gui/501/${label} >/dev/null 2>&1`); enabled = true; } catch {}
    out.push({
      session: 'launchd', project: meta.name, tool: 'launchd',
      prompt: meta.prompt, reason: '', schedule: schedule || '',
      delay: interval || 0, ageSecs: age, source: 'launchd',
      kind: 'launchd', label, enabled,
    });
  }
  return out;
}

// Live /loop view = hook heartbeats (authoritative, fresh) merged with a scan of
// open sessions' transcripts (catches loops that started before the hook existed).
let loopsCache = { ts: 0, payload: null };
app.get('/api/loops', (req, res) => {
  if (loopsCache.payload && Date.now() - loopsCache.ts < 8000) {
    return res.json(loopsCache.payload);
  }
  const now = Math.floor(Date.now() / 1000);
  const bySession = {}; // session(8) -> loop record

  // session registry: short id -> {pid, name, cwd, sessionId}
  const sessDir = path.join(os.homedir(), '.claude', 'sessions');
  let sessFiles = [];
  try { sessFiles = fs.readdirSync(sessDir).filter(f => f.endsWith('.json')); } catch {}
  const sessMeta = {};
  for (const f of sessFiles) {
    let s; try { s = JSON.parse(fs.readFileSync(path.join(sessDir, f), 'utf8')); } catch { continue; }
    const sid = s.sessionId || '';
    if (!sid) continue;
    sessMeta[sid.slice(0, 8)] = { pid: s.pid || 0, name: s.name || '', cwd: s.cwd || '', sessionId: sid };
  }

  // 1) hook heartbeats (self-cleaning)
  const hbDir = path.join(os.homedir(), '.claude', 'logs', 'loops');
  let hbFiles = [];
  try { hbFiles = fs.readdirSync(hbDir).filter(f => f.endsWith('.json')); } catch {}
  for (const f of hbFiles) {
    const fp = path.join(hbDir, f);
    let r; try { r = JSON.parse(fs.readFileSync(fp, 'utf8')); } catch { continue; }
    if ((r.next_ts || 0) + 150 < now) { try { fs.unlinkSync(fp); } catch {} continue; }
    const sid = String(r.session_id || '').slice(0, 8);
    const meta = sessMeta[sid] || {};
    bySession[sid] = {
      session: sid, project: r.project || '', tool: r.tool || '',
      prompt: r.prompt || '', reason: r.reason || '', schedule: r.schedule || '',
      delay: r.delay || 0, ageSecs: now - (r.ts || now), source: 'live',
      kind: 'session', pid: meta.pid || 0, name: meta.name || '',
    };
  }

  // 2) scan open sessions (registry pid alive) for loop tool calls
  const tIdx = sessFiles.length ? transcriptIndex() : {};
  for (const short of Object.keys(sessMeta)) {
    const meta = sessMeta[short];
    if (!meta.pid || !pidAlive(meta.pid)) continue; // closed session
    if (bySession[short]) continue; // heartbeat already covers it
    const tp = tIdx[meta.sessionId];
    if (!tp) continue;
    const call = lastLoopCall(tp);
    if (!call) continue;
    const tsec = call.ts ? Math.floor(new Date(call.ts).getTime() / 1000) : 0;
    const age = tsec ? now - tsec : 999999;
    const delay = call.input.delaySeconds || 0;
    const isCron = call.name === 'CronCreate';
    const isLive = delay && age < delay * 2 + 300;
    if (!isCron && !isLive) continue; // ended dynamic loop - skip
    bySession[short] = {
      session: short,
      project: path.basename(meta.cwd || '') || '',
      tool: call.name,
      prompt: String(call.input.prompt || '').slice(0, 200),
      reason: String(call.input.reason || '').slice(0, 200),
      schedule: String(call.input.schedule || '').slice(0, 80),
      delay, ageSecs: age, source: isCron ? 'cron' : 'scan',
      kind: 'session', pid: meta.pid, name: meta.name || '',
    };
  }

  const sessionLoops = Object.values(bySession).sort((a, b) => a.ageSecs - b.ageSecs);
  const loops = [...sessionLoops, ...launchdLoops(now)];
  const payload = { loops, count: loops.length };
  loopsCache = { ts: Date.now(), payload };
  res.json(payload);
});

// Kill / toggle a loop. Session loops -> SIGTERM the session pid (kills the
// whole session). Launchd loops -> bootout (stop) / bootstrap (start) the agent.
app.post('/api/loops/kill', (req, res) => {
  const { kind, pid, label, action } = req.body || {};
  if (kind === 'session') {
    const p = parseInt(pid, 10);
    if (!p || !pidAlive(p)) return res.status(404).json({ error: 'session not running' });
    // safety: pid must belong to a registered session
    const sessDir = path.join(os.homedir(), '.claude', 'sessions');
    let owned = false;
    try {
      for (const f of fs.readdirSync(sessDir).filter(x => x.endsWith('.json'))) {
        ...
        if (s.pid === p) { owned = true; break; }
      }
    } catch {}
    if (!owned) return res.status(403).json({ error: 'pid is not a registered Claude session' });
    try { process.kill(p, 'SIGTERM'); } catch (e) { return res.status(500).json({ error: String(e.message || e) }); }
    loopsCache = { ts: 0, payload: null };
    return res.json({ ok: true, killed: p });
  }
  if (kind === 'launchd') {
    if (!LAUNCHD_LABELS[label]) return res.status(403).json({ error: 'unknown launchd label' });
    const plist = path.join(os.homedir(), 'Library', 'LaunchAgents', label + '.plist');
    const cmd = action === 'start'
      ? `launchctl bootstrap gui/501 "${plist}"`
      : `launchctl bootout gui/501/${label}`;
    require('child_process').exec(cmd, (err, _o, stderr) => {
      loopsCache = { ts: 0, payload: null };
      if (err) return res.status(500).json({ error: (stderr || err.message || '').trim() });
      res.json({ ok: true, action: action === 'start' ? 'started' : 'stopped', label });
    });
    return;
  }
  return res.status(400).json({ error: 'kind must be session or launchd' });
});

// Change a launchd automation's StartInterval, then reload the agent.
app.post('/api/loops/interval', (req, res) => {
  const { label } = req.body || {};
  const seconds = parseInt(req.body && req.body.seconds, 10);
  if (!LAUNCHD_LABELS[label]) return res.status(403).json({ error: 'unknown launchd label' });
  if (!seconds || seconds < 60 || seconds > 86400) return res.status(400).json({ error: 'seconds must be 60-86400' });
  const plist = path.join(os.homedir(), 'Library', 'LaunchAgents', label + '.plist');
  let xml;
  try { xml = fs.readFileSync(plist, 'utf8'); } catch { return res.status(404).json({ error: 'plist not found' }); }
  if (!/<key>StartInterval<\/key>\s*<integer>\d+<\/integer>/.test(xml)) {
    return res.status(422).json({ error: 'no StartInterval in plist' });
  }
  const next = xml.replace(/(<key>StartInterval<\/key>\s*<integer>)\d+(<\/integer>)/, `$1${seconds}$2`);
  try { fs.writeFileSync(plist, next); } catch (e) { return res.status(500).json({ error: String(e.message || e) }); }
  // reload so launchd picks up the new interval
  require('child_process').exec(`launchctl bootout gui/501/${label} 2>/dev/null; launchctl bootstrap gui/501 "${plist}"`, (err, _o, stderr) => {
    loopsCache = { ts: 0, payload: null };
    if (err) return res.status(500).json({ error: (stderr || err.message || '').trim() });
    res.json({ ok: true, label, seconds });
  });
});

// Tail the log / run history for a loop.
app.get('/api/loops/log', (req, res) => {
  const { kind, label, session } = req.query;
  const N = 80;
  if (kind === 'launchd') {
    const meta = LAUNCHD_LABELS[label];
    if (!meta || !meta.log) return res.status(404).json({ error: 'no log for this job' });
    let lines = [];
    try { lines = fs.readFileSync(meta.log, 'utf8').split('\n').filter(Boolean); } catch {}
    return res.json({ lines: lines.slice(-N), source: meta.log });
  }
  if (kind === 'session') {
    const tIdx = transcriptIndex();
    const sid = Object.keys(tIdx).find(k => k.slice(0, 8) === String(session));
    if (!sid) return res.status(404).json({ error: 'transcript not found' });
    let runs = [];
    try {
      const txt = fs.readFileSync(tIdx[sid], 'utf8').split('\n');
      for (const line of txt) {
        ...
        if (!Array.isArray(content)) continue;
        for (const c of content) {
          if (c.type === 'tool_use' && (c.name === 'ScheduleWakeup' || c.name === 'CronCreate')) {
            runs.push(`${o.timestamp || '?'}  ${c.name}  ${String((c.input && (c.input.reason || c.input.prompt)) || '').slice(0, 100)}`);
          }
        }
      }
    } catch {}
    return res.json({ lines: runs.slice(-N), source: tIdx[sid] });
  }
  return res.status(400).json({ error: 'kind must be session or launchd' });
});
...
app.post('/api/crons/:id/toggle', (req, res) => {
  ...
  // auto-restart uses a JSON file, not a LaunchAgent
  if (cron.id === 'auto-restart') {
    const arFile = path.join(__dirname, 'data', 'auto-restart.json');
    let current = false;
    try { current = JSON.parse(fs.readFileSync(arFile, 'utf8')).enabled; } catch {}
    fs.writeFileSync(arFile, JSON.stringify({ enabled: !current }));
    refreshCronCache();
    return res.json({ enabled: !current, id: cron.id });
  }
